...
https://github.com/bluenviron/mediamtx[MediaMTX] accepts an input stream from the Raspberry Pi device's camera system (for example, created by `rpicam-vid`), and re-streams it to other clients. Additionally, it supports Raspberry Pi cameras as a built-in source.
...
To configure MistServer, go to the Management Interface (MI) webpage on your Raspberry Pi at `http://localhost:4242/`; you can also visit the page from another computer if you substitute the IP address of your Raspberry Pi. The first time you use the Management Interface, it asks you to create an admin account. In the steps that follow, you can accept the defaults.
