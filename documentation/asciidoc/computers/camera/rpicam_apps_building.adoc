...
Download a local copy of the Raspberry Pi fork of `libcamera` from GitHub:
...
TIP: On devices with 1 GB of memory or less, the build might exceed available memory. Append the `-j 1` flag to `ninja` commands to limit the build to a single process. This can prevent the build from exceeding available memory on devices like the Raspberry Pi Zero and the Raspberry Pi 3.
...
Download a local copy of Raspberry Pi device's `rpicam-apps` GitHub repository:
...
TIP: On devices with 1 GB of memory or less, the build might exceed available memory. Append the `-j 1` flag to `meson` commands to limit the build to a single process. This can prevent the build from exceeding available memory on devices like the Raspberry Pi Zero and the Raspberry Pi 3.
...
[TIP]
====
The preceding command automatically updates the `ldconfig` cache. If you have trouble accessing your new `rpicam-apps` build, run the following command to update the cache:
...
The output includes the date and time of your local `rpicam-apps` build.
...
`-Denable_tflite=enabled`:: Enables or disables TensorFlow Lite post-processing stages. Disabled by default. Requires Tensorflow Lite to enable. Depending on how you have built and/or installed TFLite, you might need to tweak the `meson.build` file in the `post_processing_stages` directory.
...
Rebuilding `libepoxy` isn't normally necessary because this library changes very rarely. If you do want to build it from scratch, follow the instructions below.
