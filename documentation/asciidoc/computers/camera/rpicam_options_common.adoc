...
Specify how long the application runs before closing. This value is interpreted as a number of milliseconds unless an optional suffix is used to change the unit. The suffix can be one of:
...
Forces a specific preview back end: `wayland-egl`, `egl`, `drm` or `qt`. If not specified, the application automatically selects a back end, preferring the native Wayland (`wayland-egl`) preview on Wayland desktop sessions.

Example: `rpicam-hello --preview-backend wayland-egl`
...
Alternatively, specify a camera mode by its index in the mode list shown by xref:camera_software.adoc#list-cameras[`list-cameras`], counting from 0 in the order listed, as `<index>` or `<index>:<packing>`. Packing defaults to `P` (packed).

Examples:

* `1` – the second mode in the list.
* `0:U` – the first mode in the list, unpacked.
...
Lens calibration is imperfect, so different camera modules of the same model might vary.
