...
* For other Raspberry Pi devices with a camera port, you can attach two or more cameras with a Video Mux board such as https://www.arducam.com/multi-camera-v2-1-adapter-raspberry-pi.html[this third-party product]. Since both cameras are attached to a single Unicam port, _only one camera can be used at a time_.
