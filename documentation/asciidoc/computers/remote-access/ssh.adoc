...
You now see the Raspberry Pi command prompt:
...
NOTE: If you receive a `connection timed out` error, you might have entered the wrong IP address for the Raspberry Pi. Check the xref:remote-access.adoc#ip-address[IP address of the Raspberry Pi].
...
You expect to see the files `id_rsa` and `id_rsa.pub`:
...
TIP: The command above assumes you have never before authorised any keys to access your Raspberry Pi. If you have previously added at least one key, instead add a new line containing the public key to the end of the `authorized_keys` file to preserve your existing keys.
