...
Any device connected to a Local Area Network is assigned an IP address. In order to connect to your Raspberry Pi from another machine using xref:remote-access.adoc#ssh[SSH] or xref:remote-access.adoc#vnc[VNC], you need to know the Raspberry Pi device's IP address. This is easy if you have a display connected, and there are a number of methods for finding it remotely from another machine on the network.
...
TIP: Your router's IP address is often `http://192.168.1.1`, but not always. You might find your router's address and credentials printed on a label on your router.
...
TIP: This can take up to a minute depending on your local network speed.
