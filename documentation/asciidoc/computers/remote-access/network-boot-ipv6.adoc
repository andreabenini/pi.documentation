...
2. The bootloader loads the firmware through TFTP and hands over the boot process to the firmware, passing it the details of the network.
3. The firmware loads the kernel and command line through TFTP.
4. The kernel boots the rest of the system, loading the root filesystem (rootfs) through NFS or some other mechanism.

The bootloader and firmware (stages 1 to 3) have been enhanced to support booting over IPv6.

IMPORTANT: IPv6 netboot is an *experimental alpha feature* and depending on feedback, we might need to change how it works in future. This only works on Raspberry Pi 4 and Compute Module 4.

=== How it works

To boot through IPv6 you need an updated version of the firmware (for example, `start4.elf`) and the bootloader. Using the latest release of Raspberry Pi OS and the latest stable bootloader should be sufficient.
...
NOTE: A mechanism to boot the Linux kernel with NFS through IPv6 is still to be demonstrated.
...
Modify the configuration to tell it to attempt network boot through IPv6 rather than IPv4.
