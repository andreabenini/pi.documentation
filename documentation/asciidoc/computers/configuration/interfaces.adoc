...
* <<vnc>> (virtual network computing) for remote access to your Raspberry Pi device's desktop. You can do this from the desktop and using the interactive `raspi-config` TUI.
...
Enabling VNC allows you to access your Raspberry Pi computer's desktop from another computer. You can enable VNC from the desktop, using the interactive `raspi-config` TUI.
...
Unlike https://support.arm.com/documentation/ddi0183/g/[PL011], the mini UART has no break detection, no framing errors detection, no parity bit, and no receive timeout interrupt. The mini UART also has smaller FIFOs. Combined with the lack of flow control, this makes the mini UART more prone to losing characters at higher baud rates. It's also generally less capable than PL011, mainly due to its baud rate link to the VPU clock speed. Further information about the mini UART can be found in the https://pip.raspberrypi.com/documents/RP-008249-DS[SoC peripherals document].
