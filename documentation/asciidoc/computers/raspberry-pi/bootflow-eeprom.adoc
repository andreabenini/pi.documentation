...
For more information about each boot mode, see the xref:raspberry-pi.adoc#raspberry-pi-bootloader-configuration[bootloader configuration] page. For a description of the GPU firmware files loaded by this stage. see the xref:configuration.adoc#boot-folder-contents[boot folder] page.
...
NOTE: The bootloader can also be updated before the firmware is started if a `pieeprom.upd` file is found. For more information about bootloader updates, see the xref:raspberry-pi.adoc#raspberry-pi-boot-eeprom[bootloader EEPROM] page.
