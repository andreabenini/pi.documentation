...
Each distinct Raspberry Pi model revision has a unique revision code. You can look up a Raspberry Pi device's revision code by running:
...
The last three lines show the hardware type, the revision code, and the Raspberry Pi device's unique serial number. For example:
...
NOTE: This list isn't exhaustive; there might be codes in use that aren't in this table. For best practices on using revision codes to identify boards, see the following section.
