...
****
Display Parallel Interface (DPI) displays can be connected to Raspberry Pi devices through the 40-pin general-purpose input/output (GPIO) connector as an alternative to using the dedicated Display Serial Interface (DSI) or High-Definition Multimedia Interface (HDMI) ports.
****
...
This interface is controlled by the GPU firmware and can be programmed by a user through special `config.txt` parameters and by enabling the correct Linux Device Tree overlay.
