...
Next, connect a suitable USB serial cable to your host computer (a Raspberry Pi works, although you might find that the easiest path is to use a USB serial cable, since it works out of the box without any `config.txt`` settings). Use the standard pins 6, 8, and 10 (GND, GPIO14, GPIO15) on a Raspberry Pi or Compute Module.
