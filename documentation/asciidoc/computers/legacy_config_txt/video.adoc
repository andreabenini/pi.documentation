...
NOTE: On Raspberry Pi 4, setting `hdmi_blanking=1` will not cause the HDMI output to be switched off, since this feature has not yet been implemented. This feature can cause issues when using applications which don't use the framebuffer, such as `omxplayer`.
...
If you are seeing HDMI issues (speckling, interference) then try `7`. Very long HDMI cables might need up to `11`, but values this high should not be used unless absolutely necessary.
...
Setting to `1` will remove all other modes except the ones specified by `hdmi_mode` and `hdmi_group` from the internal list, meaning they will not appear in any enumerated lists of modes. This option might help if a display seems to be ignoring the `hdmi_mode` and `hdmi_group` settings.
...
Your HDMI monitor might only support a limited set of formats. To find out which formats are supported, use the following method:
...
This might not work if your monitor doesn't support standard CVT timings.
...
Setting `sdtv_disable_colourburst` to `1` disables colourburst on composite video output. The picture is displayed in monochrome, but it might appear sharper.
...
In the latest firmware, `display_rotate` is deprecated. It has only been retained for backwards compatibility. Use `display_lcd_rotate` and `display_hdmi_rotate` instead.
...
Forces `dispmanx` composition to be done offline in two offscreen framebuffers. This can allow more `dispmanx` elements to be composited, but is slower and might limit screen framerate to around 30 fps.
