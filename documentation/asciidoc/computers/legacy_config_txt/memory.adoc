...
Specifies how much memory, in megabytes, to reserve for the exclusive use of the GPU: the remaining memory is allocated to the Arm CPU for use by the OS. For Raspberry Pi devices with less than 1 GB of memory, the default is `64`; for Raspberry Pi devices with 1 GB or more of memory the default is `76`.
...
You can also use `gpu_mem_256`, `gpu_mem_512`, and `gpu_mem_1024` to allow swapping the same SD card between Raspberry Pi devices with different amounts of RAM without having to edit `config.txt` each time:

=== `gpu_mem_256`

The `gpu_mem_256` command sets the GPU memory in megabytes for Raspberry Pi devices with 256 MB of memory. It is ignored if memory size is not 256 MB. This overrides `gpu_mem`.

=== `gpu_mem_512`

The `gpu_mem_512` command sets the GPU memory in megabytes for Raspberry Pi devices with 512 MB of memory. It is ignored if memory size is not 512 MB. This overrides `gpu_mem`.

=== `gpu_mem_1024`

The `gpu_mem_1024` command sets the GPU memory in megabytes for Raspberry Pi devices with 1 GB or more of memory. It is ignored if memory size is smaller than 1 GB. This overrides `gpu_mem`.
