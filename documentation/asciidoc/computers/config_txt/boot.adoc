...
Both forms can use the `img` file extension; the bootloader recognises archives using signature bytes at the start of the file.
...
(The reason the firmware checks for the existence of key files rather than directories when checking prefixes is twofold: the prefix can't be a directory, and not all boot methods support testing for the existence of a directory.)
...
USB On-The-Go (often abbreviated to OTG) is a feature that allows supporting USB devices with an appropriate OTG cable to configure themselves as USB hosts. On older Raspberry Pi devices, a single USB 2.0 controller was used in both USB host and device mode.
...
For more information about `secure-boot` and creating `boot.img` files, see https://github.com/raspberrypi/usbboot/blob/master/Readme.md[USBBOOT].
...
Sets the value of `boot_arg1` to be passed through a reset-safe register to the bootloader after a reboot.
...
Sets the value of xref:raspberry-pi.adoc#BOOT_ORDER[BOOT_ORDER] to be passed through a reset-safe register to the bootloader after a reboot. As with `tryboot`, this is a one-time setting and is automatically cleared after use.
...
You can set this option to 0 to block self-update without requiring the EEPROM configuration to be updated. This is sometimes useful when updating multiple Raspberry Pi devices through network boot because this option can be controlled per Raspberry Pi (for example, by using a serial number filter in `config.txt`).
...
You can use the GPIO as a general-purpose I/O pin after the OS has started. However, you should verify that this GPIO configuration does not conflict with any HATs which might pull the GPIO low during boot.
...
For safety, this OTP value can _only_ be programmed by using `RPIBOOT`. As a result, you must first clear the bootloader EEPROM using `erase_eeprom`. The blank EEPROM causes the ROM to failover to `RPIBOOT` mode, which then allows this option to be set.
