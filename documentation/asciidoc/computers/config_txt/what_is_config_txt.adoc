...
The `config.txt` file is read by the early-stage boot firmware, so it uses a very simple file format: **a single `property=value` statement on each line, where `value` is either an integer or a string**. You can add comments, or comment out existing config values, by starting a line with the `#` character.
