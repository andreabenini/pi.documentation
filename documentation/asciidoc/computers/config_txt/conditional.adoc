...
However, if one Raspberry Pi is swapped between different monitors, or if the SD card (or card image) is being swapped between multiple boards, a single set of settings might no longer be sufficient. Conditional filters allow you to define certain sections of the config file to be used only in specific cases, allowing a single `config.txt` to create different configurations when read by different hardware.
...
The raw value of the `PM_RSTS` register at bootup is available through `/proc/device-tree/chosen/bootloader/rsts` and the final partition number used for booting is available through `/proc/device-tree/chosen/bootloader/partition`. These are big-endian binary values.
...
* Integer constants can either be written as decimal or as hex.
...
Setting `boot_arg1` to 42 in `config.txt`:
[source,ini]
----
set_reboot_arg1=42
----
...
Setting `boot_arg1` to 42 by using `vcmailbox``:
[source,console]
----
sudo vcmailbox 0x0003808c 8 8 1 42
----

Reading `boot_arg1` by using `vcmailbox``:
[source,console]
----
...
----
The value of the `boot_arg1` variable when the OS was started can be read from xref:configuration.adoc#part4[device-tree] at `/proc/device-tree/chosen/bootloader/arg1`
...
For example, setting `bootvar0` to 42 in `rpi-eeprom-config`:
[source,ini]
----
BOOTVAR0=42
----
...
To read `boot_count` by using `vcmailbox`:
[source,console]
----
...
Setting/clearing `boot_count` by using `vcmailbox`:
[source,console]
----
# Clear boot_count by setting it to zero.
sudo vcmailbox 0x0003808d 4 4 0
----
The value of `boot_count` when the OS was started can be read from xref:configuration.adoc#part4[device-tree] at `/proc/device-tree/chosen/bootloader/count`
...
==== `partition`
The `partition` variable can be used to select alternate boot flows according to the requested partition number (`sudo reboot N`) or through direct usage of the `PM_RSTS` watchdog register.
...
The value of `tryboot` at the start of boot can be read from xref:configuration.adoc#part4[device-tree] at `/proc/device-tree/chosen/bootloader/tryboot`
...
A 16-digit hex value is displayed near the bottom of the output. The serial number of your Raspberry Pi is the last eight hex-digits. For example, if you see:
