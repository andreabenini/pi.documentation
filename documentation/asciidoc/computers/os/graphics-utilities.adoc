...
To query a specific interface on Raspberry Pi 5, you might need to specify the device node using the `--device` (or `-d`) option:
...
Card numbers are assigned dynamically at boot and can change between restarts depending on the attached hardware.
