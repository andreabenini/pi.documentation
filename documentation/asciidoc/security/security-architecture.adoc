...
We recommend provisioning your devices with secure boot using Raspberry Pi Secure Boot Provisioner, with optional disk encryption enabled. For information and instructions, see xref:../security/secure-boot-provisioner.adoc[Secure Boot Provisioner].
...
The first stage is the *boot ROM* embedded in each Raspberry Pi device's SoC. Because it's permanently burned into silicon, it can't be modified after manufacturing and is trusted by design; it thus forms the hardware root of trust for the secure boot process.
...
The second stage is the *EEPROM bootloader*, which is stored in the Raspberry Pi device's SPI flash EEPROM and updated by Raspberry Pi Secure Boot Provisioner. The digital signature of this bootloader is verified using Raspberry Pi's public key embedded in the boot ROM. The EEPROM bootloader then checks the customer's public key against the hash stored in OTP, ensuring that only an authorised customer key is trusted for subsequent stages.
