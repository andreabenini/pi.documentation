A firewall is a network security system that monitors and controls incoming and outgoing network traffic based on a defined set of security rules. This page introduces *Uncomplicated Firewall (UFW)*, a firewall tool that simplifies firewall configuration on Linux systems using https://www.netfilter.org/projects/iptables/index.html[`iptables`].
...
System administrators configure netfilter using https://www.netfilter.org/projects/iptables/index.html[`iptables`], which is the traditional command-line tool for defining packet-filtering rules. However, `iptables` can be complex. https://www.linux.com/learn/introduction-uncomplicated-firewall-ufw[UFW (Uncomplicated Firewall)], the firewall tool used by Ubuntu, simplifies firewall configuration and management. It acts as a wrapper for `iptables`, allowing system administrators to set firewall rules without dealing with the complexity of raw `iptables` commands.
...
For example, `sudo ufw --dry-run allow 22` shows you the predicted outcome of allowing traffic on port 22, including the entire set of rules if the change were made.
