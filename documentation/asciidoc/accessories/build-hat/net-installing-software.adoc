...
The .NET framework from Microsoft is not available through `apt` on Raspberry Pi. However, you can follow the https://docs.microsoft.com/en-us/dotnet/iot/deployment[official instructions] from Microsoft to install the .NET framework. Alternatively, there is a simplified https://www.petecodes.co.uk/install-and-use-microsoft-dot-net-5-with-the-raspberry-pi/[third party route] to get the .NET toolchain on to your Raspberry Pi.

WARNING: The installation script is run as `root`. Before running it, read it and make sure you understand what it is doing. If you're at all unsure, follow the https://docs.microsoft.com/en-us/dotnet/iot/deployment[official instructions] manually.
...
You can run the program with the `dotnet run` command. Let's try it now to make sure everything works. It prints "Hello World!"
...
(When instructed to "run the program" in the instructions that follow, rerun `dotnet run`)
