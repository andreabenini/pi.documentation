...
We recommend using your xref:../computers/raspberry-pi.adoc#gpio[Raspberry Pi device's GPIO header] to provide power to the Touch Display.
...
. Connect jumper wires between the 5 V and `GND` pins on the xref:../computers/raspberry-pi.adoc#gpio[Raspberry Pi device's GPIO] and the 5 V and `GND` pins on the display.
+
...
NOTE: To reverse this action, use an SSH connection (such as Raspberry Pi Connect's remote access), an HDMI display, or a separate computer with the Raspberry Pi device's boot media attached.
