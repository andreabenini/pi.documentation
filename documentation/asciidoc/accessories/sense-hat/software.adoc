...
After it starts, you see output similar to the following scrolling up the screen:
...
NOTE: On Raspberry Pi 2 Model B Rev 1.0 and Raspberry Pi 3 Model B boards, these steps might not work. The firmware will take control of I2C0, causing the ID pins to be configured as inputs.
...
NOTE: This operation will not damage your Raspberry Pi or Sense HAT, but if an error occurs, your Raspberry Pi might fail to automatically detect the HAT.
