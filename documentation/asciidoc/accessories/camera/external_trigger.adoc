...
WARNING: This modification includes removing an SMD soldered part. Don't attempt this modification unless you feel you are competent to complete it. When soldering to the Camera board, remove the plastic back cover to avoid damaging it.
...
We can use a Raspberry Pi Pico to provide the trigger. Connect any Pico GPIO pin (GP28 is used in this example) to XTR through a 1.5 kΩ resistor. Also connect a 1.8 kΩ resistor between XTR and GND to reduce the high logic level to 1.8 V. A wiring diagram is shown below.
...
Every time the Pico pulses the pin, it attempts to capture a frame. However, if `--gain` and `--awbgains` are not set, some frames are dropped to allow AGC and AWB algorithms to settle.
